...
#Region " Structures "
    ...
    Public Structure GPSPlayerEntry
        Dim Name As String
        Dim Loc As ITibia.LocationDefinition
        Dim HealthPoints As Integer
...
#Region " Objects "
        ...
        Public WithEvents AutoResponderForm As New frmAutoResponder
        ...
        Public WithEvents GPSBroadcasterTimerObj As ThreadTimer
        ...
        Public WithEvents GPSMapForm As New frmGPSMap
        Public WithEvents HUDTimerObj As ThreadTimer
        Public WithEvents LootOpenTibiaServerTimerObj As ThreadTimer
#End Region
...
#Region " Variables "
        ...
        Public GPSChannelName As String = ""
        Public GPSChannelPassword As String = ""
        Public GPSBroadcastStats As Boolean = False
        Public GPSBroadcastBattlelist As Boolean = False
        Public GPSPlayerEntries As New SortedList(Of String, GPSPlayerEntry)
        ...
        Public AntiLogoutMoveNow As Boolean = False
        Public WaitForLootMonster As New Date
...
#Region " Initialization"
        ...
        Public Sub New()
            Try
                ...
                GPSBroadcasterTimerObj = New ThreadTimer(5000)
                ...
                LootOpenTibiaServerTimerObj = New ThreadTimer(700)
        ...
        Private Sub LootMonster()
            Try
                ...
                If IsOpenTibiaServer Then
                    WaitForLootMonster = Date.Now.AddMilliseconds(1200)
                    LootOpenTibiaServerTimerObj.StartTimer(800)
                    Exit Sub
                End If
...
#Region " Thread Timers "
        Public Sub StopEverything()
            Try
                ...
                GPSBroadcasterTimerObj.StopTimer()
                GPSChannelName = ""
                GPSChannelPassword = ""
                GPSBroadcastStats = False
                GPSBroadcastBattlelist = False
                GPSChannelName = ""
                PublicMessageFile = ""
                PrivateMessageFile = ""
                Thread.Sleep(500)
...
#Region " Auto Looter Timer "
        ...
        Private Sub LootOpenTibiaServerTimerObj_OnExecute() Handles LootOpenTibiaServerTimerObj.OnExecute
            Try
                SyncLock LootOpenTibiaServerTimerObj
                    If LootOpenTibiaServerTimerObj.State = IThreadTimer.ThreadTimerState.Stopped Then Exit Sub
                    Dim N As Byte = 0
                    'N = Container.ContainerCount
                    'If N > &HF Then N = &HF
                    N = &HE
                    'Dim buffer() As Byte
                    While Date.Now < WaitForLootMonster
                    ...
                    LooterItemID = 0
                    If Kernel.CaveBotTimerObj.State = IThreadTimer.ThreadTimerState.Running Then
                        Dim WaitMoreTime As Date = Date.Now.AddMilliseconds(400)
                        While Date.Now < WaitMoreTime
                            System.Threading.Thread.Sleep(100)
                        End While
                        WaitTime = Date.Now.AddSeconds(8)
                        IsOpeningReady = True
                        CBCreatureDied = False
                    End If
                    LootOpenTibiaServerTimerObj.StopTimer()
                End SyncLock
            Catch ex As Exception
                LootOpenTibiaServerTimerObj.StopTimer()
            End Try
                    ...
                    Case CavebotState.OpeningBody
                        ...
                        If IsOpeningReady = True Then
                            ...
                            If CurrentContCount > CBContainerCount Then
                                WaitAttacker = Nothing
                                CBState = CavebotState.Looting
                                'ConsoleWrite("Looting state ->")
                                If IsOpenTibiaServer Then
                                    System.Threading.Thread.Sleep(500)
                                End If
                            Else
                                If Date.Now > WaitTime Then
                                    'ConsoleWrite("Running out of time, Walking ->")
                                    WaitAttacker = Nothing
                                    CBState = CavebotState.Walking
                                End If
                            End If
                ...
                For XAxis As Integer = 1 To 15
                    For YAxis As Integer = 1 To 11
                        ...
                        For Each TileObject In TileObjects
                            ...
                            OrElse Client.Objects.LensHelp(TileObjectID) = IObjects.ObjectLensHelp.StonePile _
...
#Region " Anti-Logout Timer "
        ...
        Public Sub AntiLogoutObj_Execute() Handles AntiLogoutObj.OnExecute
            Try
                SyncLock AntiLogoutObj

                    If Not Kernel.Client.IsConnected Then Exit Sub
                    Dim IdleTime As TimeSpan = Date.Now.Subtract(LastActivity)
                    If IdleTime.TotalMinutes < 13 Then
                        If AntiLogoutMoveNow = False Then Exit Sub
                    End If

                    Dim BL As New BattleList
                    Dim MyLastDirection As Scripting.IBattlelist.Directions
...
#Region " IL Broadcaster Timer "
        Private Sub ILBroadcasterTimerObj_OnExecute() Handles GPSBroadcasterTimerObj.OnExecute
            Try
                If IRCClient.IsHiddenChannelOpened(GPSChannelName) Then
                    Dim IPB As New IrcPacketBuilder(Kernel.IRCClient, GPSChannelName)
                    IPB.BroadcastLocation(Client.CharacterName, Client.CharacterLocation)
                    If GPSBroadcastStats Then
                        IPB.BroadcastStats(Client.CharacterName, Client.CharacterLevel, Client.CharacterHitPoints, Client.CharacterManaPoints)
                    End If
                    Thread.Sleep(125)
                    If GPSBroadcastBattlelist Then
                        Dim BL As New BattleList()
                        Dim EntityLoc, CharLoc As ITibia.LocationDefinition

                        BL.Reset(True)
                        Do
                            If BL.IsMyself Then Continue Do
                            CharLoc = Client.CharacterLocation
                            EntityLoc = BL.GetLocation
                            If Abs(EntityLoc.X - CharLoc.X) <= 8 AndAlso _
                                Abs(EntityLoc.Y - CharLoc.Y) <= 6 AndAlso _
                                Abs(EntityLoc.Z - CharLoc.Z) <= 1 AndAlso _
                                BL.GetHPPercentage > 0 Then
                                IPB.BroadcastEntityStats(BL.GetName, BL.GetHPPercentage, EntityLoc)
                                Thread.Sleep(125)
                            End If
                        Loop While BL.NextEntity(True)
                    End If
...
#Region " IRC Events "
            ...
            Try
                If IrcChannelIsOpened(Channel) Then
                    ...
                    If Message.ToLower.Contains(IRCClient.Nick.ToLower) Then
                        ...
                        If Consts.IRCPlaySoundIfHighlight Then
                            Dim Sound As New Audio
                            Sound.Play(Kernel.ExecutablePath & "\Alarms\" & "Pinc" & ".wav", AudioPlayMode.Background)
                        End If
                    End If
                End If
            Catch Ex As Exception
        ...
        Public Sub IrcClient_PacketReceived(ByVal Nick As String, ByVal Channel As String, ByVal Packet As String) Handles IRCClient.PacketReceived
            Try
                Dim M As Match = Regex.Match(Packet, "^\$([^\$]*)\$([^\$]+)\$$")
                If M.Success Then
                    Dim Destinatary As String = M.Groups(1).Value
                    Dim P() As Byte = System.Convert.FromBase64String(M.Groups(2).Value)
                    Dim Pos As Integer = 2
                    Select Case GetByte(P, Pos)
                        Case &H1 ' broadcast location
                            Dim CharacterName As String = GetString(P, Pos)
                            Dim Loc As ITibia.LocationDefinition = GetLocation(P, Pos)
                            'ConsoleWrite("Location: " & CharacterName & " (" & Loc.X & "," & Loc.Y & "," & Loc.Z & ")")
                            SyncLock GPSPlayerEntries
                                If GPSPlayerEntries.ContainsKey(CharacterName) Then
                                    Dim GE As GPSPlayerEntry = GPSPlayerEntries(CharacterName)
                                    GE.Loc = Loc
                                    GE.LastUpdate = DateTime.Now
                                    GPSPlayerEntries(CharacterName) = GE
                                Else
                                    Dim GE As New GPSPlayerEntry
                                    GE.Name = CharacterName
                                    GE.Loc = Loc
                                    GE.LastUpdate = DateTime.Now
                                    GPSPlayerEntries.Add(CharacterName, GE)
                                End If
                            End SyncLock
                        Case &H2 ' broadcast stats
                            Dim CharacterName As String = GetString(P, Pos)
                            Dim Level As UInteger = GetDWord(P, Pos)
                            Dim HealthPoints As UInteger = GetDWord(P, Pos)
                            Dim ManaPoints As UInteger = GetDWord(P, Pos)
                            SyncLock GPSPlayerEntries
                                If GPSPlayerEntries.ContainsKey(CharacterName) Then
                                    Dim GE As GPSPlayerEntry = GPSPlayerEntries(CharacterName)
                                    GE.LastUpdate = DateTime.Now
                                    GE.Level = Level
                                    GE.HealthPoints = HealthPoints
                                    GE.ManaPoints = ManaPoints
                                    GPSPlayerEntries(CharacterName) = GE
                                Else
                                    Dim GE As New GPSPlayerEntry
                                    GE.Name = CharacterName
                                ...
                                End If
                            End SyncLock
                            'ConsoleWrite("Stats """ & CharacterName & """ Lv." & Level & " HP:" & HealthPoints & " MP: " & ManaPoints)
                        Case &H3 ' broadcast entity
                            Dim CharacterName As String = GetString(P, Pos)
                            Dim HPPercent As Byte = GetByte(P, Pos)
                            Dim Loc As ITibia.LocationDefinition = GetLocation(P, Pos)
                            SyncLock GPSPlayerEntries
                                If GPSPlayerEntries.ContainsKey(CharacterName) Then
                                    Dim GE As GPSPlayerEntry = GPSPlayerEntries(CharacterName)
                                    GE.LastUpdate = DateTime.Now
                                    GE.HPPercent = HPPercent
                                    GE.Loc = Loc
                                    GPSPlayerEntries(CharacterName) = GE
                                Else
                                    Dim GE As New GPSPlayerEntry
                                ...
                                End If
                            End SyncLock
                            'ConsoleWrite("Entity """ & CharacterName & """ " & HPPercent & "% (" & Loc.X & "," & Loc.Y & "," & Loc.Z & ")")
                    End Select
                Else
...
#Region " Packets from Client Parsers "
        ...
        Private Sub ClientParseUseItem(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
            ...
            If LearningMode Then
                If Client.Objects.LensHelp(ItemID) = IObjects.ObjectLensHelp.Ladder Then
                    ...
                    AutoAddTime = Now.AddSeconds(5)
                End If
                If Client.Objects.LensHelp(ItemID) = IObjects.ObjectLensHelp.SewerGrates Then
                    Dim WalkerChar As New Walker
                    WalkerChar.Coordinates = Location
        ...
        Private Sub ClientParseUseItemWith(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
        ...
        Private Sub ClientParseUseHotKey(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
            Pos += 13
        End Sub

        Private Sub ClientParseGoToParent(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
            Dim ContainerIndex As Integer = GetByte(bytBuffer, Pos)
            Dim MyContainer As New Container
        ...
        'idk how call this >.<
        Private Sub ClientParseGetDataFromDialog(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
        ...
        Private Sub ClientParseSendMessage(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
        ...
        Private Sub ClientParseChannelList(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
        ...
        Private Sub ClientParseCloseChannel(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
        ...
        Private Sub ClientParseOpenChannel(ByRef bytBuffer() As Byte, ByRef Pos As Integer, ByRef Send As Boolean)
...
#End Region
            ...
            Try
                ...
                Select Case ID
                    ...
                    Case &H83 'Use Item With
                        ClientParseUseItemWith(bytBuffer, Pos, Send)
                        Proxy.LastAction = Date.Now.Ticks
                        LastActivity = Date.Now
                    Case &H84 'Use hotkey
                        ClientParseUseHotKey(bytBuffer, Pos, Send)
                        Proxy.LastAction = Date.Now.Ticks
                        LastActivity = Date.Now
                    Case &H88 'go to parent
                        ClientParseGoToParent(bytBuffer, Pos, Send)
                        Proxy.LastAction = Date.Now.Ticks
                        LastActivity = Date.Now
                    Case &H8A
                    ...
                    Case &H98 ' Requesting console through Channel List
                        ClientParseChannelList(bytBuffer, Pos, Send)
                        Proxy.LastAction = Date.Now.Ticks
                        LastActivity = Date.Now
                    Case &H99 ' Closing channel
                        ClientParseCloseChannel(bytBuffer, Pos, Send)
                        Proxy.LastAction = Date.Now.Ticks
                        LastActivity = Date.Now
                    Case &H9A ' Requesting console given a string
                        ClientParseOpenChannel(bytBuffer, Pos, Send)
                        Proxy.LastAction = Date.Now.Ticks
                        LastActivity = Date.Now
        ...
        Private Sub Proxy_PacketFromServer(ByRef bytBuffer() As Byte, ByRef Send As Boolean) Handles Proxy.PacketFromServer
                ...
                While Pos < PacketLength
                    ...
                    Select Case PacketID
                        ...
                        Case &HB4 'sys message ^_^u
                            ...
                            Dim Type As ITibia.SysMessageType = GetByte(bytBuffer, Pos)

                            Dim Text As String = GetString(bytBuffer, Pos)
                            'Kernel.ConsoleRead("SysMsg: " & Text & "::: Type: " & Type)
                            If Type = ITibia.SysMessageType.Information Then
                                ...
                                If EntityInfoDismissLookPacket Then
                                    ...
                                    If PacketLength - 2 = PLength Then
                                        Send = False
                                        Exit Sub
                                    End If
                                    Dim bytBufferTemp() As Byte = bytBuffer
                                    ReDim bytBuffer(1)
                                    For I As Integer = 2 To PacketLength - 1
                                        If I >= InitPos AndAlso I < EndPos Then
                                            Continue For
                                        End If
                                        AddByte(bytBuffer, bytBufferTemp(I))
                                    Next
                                ...
                                Else
                                    ...
                                    End If
                                End If
                            ElseIf Type = ITibia.SysMessageType.StatusWarning Then
                                'Lets move
                                If Text.Contains("You have been idle for 15 minutes") Then
                                    If Kernel.AntiLogoutObj.State = IThreadTimer.ThreadTimerState.Stopped Then Exit Sub
                            ...
                            ElseIf Client.Objects.HasFlags(ID, IObjects.ObjectFlags.IsContainer) Then
                                ...
                                If LooterTimerObj.State = IThreadTimer.ThreadTimerState.Running Then
                                    If String.IsNullOrEmpty(Client.Objects.Name(ID)) Then 'if its known container, skip
                                        Dim BL As New BattleList
                                        BL.JumpToEntity(IBattlelist.SpecialEntity.Myself)
                                        If BL.GetDistanceFromLocation(Loc) <= Consts.LootMaxDistance AndAlso Loc.Z = Client.CharacterLocation.Z Then
                                            If CaveBotTimerObj.State = IThreadTimer.ThreadTimerState.Running Then
                                                ...
                                                If IsOpenTibiaServer Then
                                                    WaitTime = Date.Now.AddSeconds(7)
                                                Else
                                                    WaitTime = Date.Now.AddSeconds(5)
                                                End If
                        ...
                        Case &HD4 'viplogout
                            Pos += 4
                            'Case &H65
                            'Trace.WriteLine("FromServer: Packet 65=" & BytesToStr(bytBuffer, Pos - 1))
                        Case Else
#If TRACE Then
...
#End If
                ...
                End While
            Catch Ex As Exception
                Trace.WriteLine("Error Reading Packet From Server: " & BytesToStr(bytBuffer, Pos - 1))
                ShowError(Ex)
                'End
            End Try
        End Sub
...
#Region " Properties "
        ...
        Public Property TTBotState() As Integer Implements IKernel.BotState
            Get
                Return Kernel.TTBState
            End Get
            Set(ByVal value As Integer)
                Kernel.TTBState = value
            End Set
        End Property
