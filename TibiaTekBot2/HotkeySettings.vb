...
#Region " Saving/Writing "
        ...
        Public Function Save(Optional ByVal Filename As String = "") As Boolean
            Try
                If String.IsNullOrEmpty(Filename) Then Filename = Kernel.GetProfileDirectory & "\" & "HotkeySettings.xml"
        ...
        Private Sub WriteToMemory()
            Try
                For I As Integer = 0 To Consts.HotkeyMax - 1
                    Kernel.Client.WriteMemory(Consts.ptrHotkeyBegin + (Consts.HotkeyItemDist * I), Hotkeys(I).UseMode, 1)
                    Select Case Hotkeys(I).Type
                        ...
                        Case HotkeyType.None
                            ...
                            Kernel.Client.WriteMemory(Consts.ptrHotkeyBegin + (Consts.HotkeyTextAutoSendDist * I) + Consts.HotkeyTextAutoSendOffset, 0, 1)
                            Kernel.Client.WriteMemory(Consts.ptrHotkeyBegin + (Consts.HotkeyTextDist * I) + Consts.HotkeyTextOffset, "")
                    End Select
                Next
...
#Region " Loading/Reading "
        ...
        Public Sub LoadFromMemory()
            Try
                ...
                Dim Text As String = ""
                For I As Integer = 0 To Consts.HotkeyMax - 1
                    Kernel.Client.ReadMemory(Consts.ptrHotkeyBegin + (Consts.HotkeyItemDist * I), UseMode, 1)
                    Hotkeys(I).UseMode = CType(UseMode, HotkeyUseMode)
                    Kernel.Client.ReadMemory(Consts.ptrHotkeyBegin + (Consts.HotkeyTextDist * I) + Consts.HotkeyTextOffset, Text)
                    Kernel.Client.ReadMemory(Consts.ptrHotkeyBegin + (Consts.HotkeyItemDist * I) + Consts.HotkeyItemOffset, ItemID, 2)
                    If ItemID > 0 Then 'item
                        Hotkeys(I).Type = HotkeyType.Item
                        Hotkeys(I).ItemID = CUShort(ItemID)
                        Kernel.Client.ReadMemory(Consts.ptrHotkeyBegin + (Consts.HotkeyItemDist * I) + Consts.HotkeyItemDataOffset, ItemData, 4)
                        Hotkeys(I).ItemData = ItemData
                    ElseIf Text.Length > 0 Then 'text
                        Hotkeys(I).Type = HotkeyType.Text
                        Hotkeys(I).Text = Text
                        Kernel.Client.ReadMemory(Consts.ptrHotkeyBegin + (Consts.HotkeyTextAutoSendDist * I) + Consts.HotkeyTextAutoSendOffset, AutoSend, 1)
