    ...
    Public Class TileData
        ...
        Public Sub Get_TileInfo()
            Try
                ...
                Kernel.Client.ReadMemory(Consts.ptrMapPointer, Map_Begin, 4)
                MapTile_Address = Map_Begin + (TileNum * Consts.MapTileDist) + 4

                'Getting info
                Kernel.Client.ReadMemory(MapTile_Address - 4, Count, 2)
                If Count > 9 Then Exit Sub
                Kernel.Client.ReadMemory(MapTile_Address, TileId, 2)
                Pos.X = Tile_Coords(Coords.X)
                Pos.Y = Tile_Coords(Coords.Y)
                Pos.Z = Tile_Coords(Coords.Z)

                For N As Integer = 0 To Count - 1
                    Kernel.Client.ReadMemory(MapTile_Address + (Consts.MapObjectDist * N) + (Consts.MapObjectIdOffset - 4), ObjectId(N), 4)
                    Kernel.Client.ReadMemory(MapTile_Address + (Consts.MapObjectDist * N) + (Consts.MapObjectDataOffset - 4), ObjectInfo(N), 4)
                Next N
                            ...
                            If Count > 1 Then
                                For E As Integer = 0 To Count - 1
                                    If ObjectId(E) = &H63 Then
                                        If ObjectInfo(E) = Kernel.CharacterID Then
